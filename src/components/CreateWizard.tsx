...
export default function CreateWizard() {
  ...
  // If the user lowers the number of nights after picking recipes, drop the
  // excess selections instead of showing "-2 will be generated by AI"
  useEffect(() => {
    setReusedIds((prev) => (prev.length > numNights ? prev.slice(0, numNights) : prev));
  }, [numNights]);
  ...
  const handleCreate = async () => {
    ...
    try {
      const res = await fetch("/api/weeks", {
        ...
        body: JSON.stringify({
          ...
          preferences: {
            ...
            leftovers: leftovers.some((l) => l.name.trim())
              ? leftovers.filter((l) => l.name.trim())
              : undefined,
            seasonal,
          },
      ...
      // Week was created; if matching was skipped the detail page shows a
      // banner with a retry — but surface a 2FA prompt right away
      if (data.picnic_matching === "2fa_required") {
        window.dispatchEvent(new CustomEvent("picnic:2fa-required"));
      }
